...
function extractNameFromTemplate(value) {
  const res = value.split(', ')[1].replace('!', '');
  return res;
}
...
function encodeToRot13(str) {
  const inputArray = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.split('');
  const outPutArray = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split('');
  const index = (x) => inputArray.indexOf(x);
  const translate = (x) => (index(x) > -1 ? outPutArray[index(x)] : x);
